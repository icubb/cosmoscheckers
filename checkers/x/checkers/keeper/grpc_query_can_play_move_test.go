...
canPlayTestRange = []canPlayGameCase{
	...
	},
	{
		...
		response: &types.QueryCanPlayMoveResponse {
			Possible: false,
			Reason: "message creator is not player: u",
		},
		err: "nil",
	},
	{
		...
		response: &types.QueryCanPlayMoveResponse {
			Possible: false,
			Reason: "player tried to play our of turn: red",
		},
		err: "nil",
	},
	{
...
// Note how all test cases are run with a singl unit test. In other words, the keeper used for 
// second use case is the same as that used for the first case, and so on for all. So to 
// mitigate the risk of interference from one case to the next, you ought to do 
// `keeper.RemoveStoredGame(ctx, testCase.game.Index)` at the end of the test case. 
