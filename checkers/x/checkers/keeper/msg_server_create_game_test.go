...
// This initialises the genesis properly so there is a next id to read from.
...
// So what has been done here is a test function to setup the mseage server on genesis.
// Creates teh context and keepre then initialises the checkers game with the defaul genesis.
// then returns these.

// the game index is "" currently because it is not implemented to return anything as of yet.

func TestCreate1GameHasSaved(t *testing.T) {
	// setup for the create game
	...
	//check if the system info has been created
	systemInfo, found := keeper.GetSystemInfo(sdk.UnwrapSDKContext(context))
	require.True(t, found)
	// check if the next id has been creatd
	...
	// get the stored game then check if it
...
func TestCreate1GameEmitted(t *testing.T) {
	//sets up a new game context next ide etc. then creaqts a game
	...
	ctx := sdk.UnwrapSDKContext(context)
	// makes sure the context is not nil
	require.NotNil(t, ctx)
	// gets teh events from the  event
	events := sdk.StringifyEvents(ctx.EventManager().ABCIEvents())
	require.Len(t, events, 1)
	event := events[0]
	// require the events equal the events below.
